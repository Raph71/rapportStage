...
d'infrastructure sont surveillées via CloudWatch, les métriques d'application via AMP \cite{aws-amp}
et les collecteurs OpenTelemetry. Les règles d'alerte, décrites en Terraform \cite{terraform-docs},
génèrent des promrules Prometheus \cite{prometheus-docs}. Alertmanager \cite{alertmanager-docs}
regroupe et route les alertes vers un topic SNS, qui alimente une file SQS. Un outil de routage se
connecte à cette file pour appliquer la logique métier de notification. L'ensemble reste indépendant
de l'outil de routage final, ce qui préserve la modularité de la solution.
