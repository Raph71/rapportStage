...
simultanés aux heures de pointe. Cette liquidité constitue un avantage concurrentiel déterminant :
plus les joueurs sont nombreux, plus l'offre de tables et de tournois est riche, ce qui renforce à son
tour l'attractivité de la plateforme. Le principal concurrent historique sur le poker reste PokerStars,
davantage implanté sur les marchés internationaux.

Sur le marché des paris sportifs, Winamax s'est hissé en tête des opérateurs français à la suite de l'Euro
de football et détient plus d'un tiers du marché. Ses principaux concurrents sont Betclic, Unibet et
Parions Sport (FDJ). La qualité des cotes proposées, régulièrement distinguée, participe de ce
positionnement. Winamax poursuit par ailleurs une stratégie d'expansion européenne, avec une
position déjà dominante sur le poker en Espagne et une progression sur le marché allemand des paris
sportifs.
...
la problématique de l'alerting : lorsqu'un incident survient, il faut pouvoir notifier précisément l'équipe
responsable du service réellement impacté, et non diffuser une alerte générique et peu
contextualisée.

\section{L'équipe Infrastructure}

L'équipe Infrastructure est une équipe transverse et essentielle de Winamax. En lien avec le directeur
technique (CTO), elle décide et met en œuvre l'orientation globale de l'entreprise en matière de choix
technologiques, comme la migration vers AWS décidée en 2018. Sa philosophie consiste à s'appuyer
au maximum sur les services gérés fournis par AWS, afin que les équipes puissent se concentrer sur le
développement de leurs produits, et à généraliser l'Infrastructure as Code (IaC), notamment via
CloudFormation, Terraform et Puppet.
...
détection d'un incident à la notification de la bonne personne.
...
Prelive, Développement, PCIDSS-prod, PCIDSS-dev, Tools, Network, Keys, IT, Stacksets, Users, entre
autres.

L'ensemble des droits IAM sur ces comptes est géré par l'équipe Sécurité, et l'accès aux
environnements se fait via des VPN reposant sur le protocole OpenVPN. Côté collaboration et
développement, les équipes s'appuient sur Google Workspace pour la bureautique et la messagerie,
Redmine pour la gestion des tickets, Slack pour la communication interne et GitHub pour
l'hébergement du code source. Slack joue un rôle central dans la problématique d'alerting, puisqu'il
constitue le principal canal de notification des équipes.

Sur le plan de l'observabilité, la plateforme combine plusieurs briques : Amazon CloudWatch pour les
métriques et les logs d'infrastructure, Amazon Managed Service for Prometheus (AMP) pour les
...
obligations fortes en matière de traçabilité des opérations de jeu, de protection des joueurs et de
sécurité des données, notamment financières. C'est cette contrainte réglementaire qui explique le
maintien de certains serveurs en datacenter, hors du cloud public, ainsi que l'existence de comptes
AWS dédiés et cloisonnés à la norme de sécurité des données de l'industrie des cartes de paiement
(PCIDSS-prod, PCIDSS-dev), séparés du reste de l'infrastructure.

Au-delà de la conformité réglementaire, Winamax opère une plateforme sollicitée en continu par des
millions de joueurs, avec des pics d'activité marqués lors des grands événements sportifs. La
disponibilité du service est donc un enjeu direct pour l'entreprise : la moindre indisponibilité se traduit
immédiatement par une dégradation de l'expérience utilisateur, une perte de revenus et un risque de
réputation. C'est dans ce contexte d'exigence réglementaire et opérationnelle que s'inscrit le rôle de
l'équipe Infrastructure, et plus particulièrement la nécessité de disposer d'un système d'alerting fiable,
capable de détecter rapidement tout incident susceptible d'affecter la disponibilité ou la conformité
de la plateforme.
