...
de stockage clé-valeur compatible Redis, issu d'un fork open source de ce dernier — pour l'équipe
PKD. J'ai réalisé cet ajout en Infrastructure as Code avec Terraform, conformément aux pratiques de
l'équipe : définition de la ressource, paramétrage réseau (sous-réseaux, groupes de sécurité),
intégration dans l'environnement cible, puis soumission via Pull Request pour revue et application par
la CI/CD. Cette tâche m'a permis d'appliquer concrètement les principes de l'IaC et de me familiariser
avec le cycle de provisionnement d'un datastore au sein de l'organisation AWS de Winamax.
...
filtrage (groupes de sécurité, ACL), la résolution DNS et la configuration des VPN. Ces interventions
m'ont appris à diagnostiquer un problème de bout en bout et à raisonner sur la connectivité entre
services répartis sur plusieurs environnements.
...
pointaient encore vers un ancien master Puppet hébergé en dehors d'AWS ; je les ai reconfigurés pour
qu'ils pointent désormais vers le master hébergé sur AWS. L'objectif était de centraliser la gestion de
configuration afin de rendre les maintenances plus simples et plus homogènes. Cette migration a
impliqué de comprendre le fonctionnement de Puppet comme outil de gestion de configuration, de
reconfigurer les agents sur les serveurs concernés, de valider que la nouvelle communication avec le
master s'établissait correctement et de s'assurer de l'absence de régression sur les configurations
appliquées. Au-delà de l'aspect technique, cette tâche illustre bien la valeur du rôle de fast : contribuer
à la dette technique et à la simplification de l'infrastructure, au service de la maintenabilité à long
terme.
