...
communiquent entre eux via des API au format JSON-RPC : un protocole simple où un service en appelle
un autre en lui envoyant un message structuré en JSON (le format texte standard d'échange de données
sur le web). Toutes ces requêtes traversent un \emph{middleware} — une couche logicielle commune,
placée sur le chemin de chaque appel, qui les traite de façon transparente sans que chaque équipe ait à
la réécrire.

Ce middleware est \emph{instrumenté} avec OpenTelemetry, un standard d'observabilité : à chaque
requête, il produit automatiquement de la télémétrie qui décrit ce qui s'est passé. Cette instrumentation
n'est pas, en soi, une métrique ; c'est à partir d'elle que sont ensuite calculées des métriques
applicatives — nombre de requêtes, taux d'erreur, temps de réponse — qui sont exportées vers AMP
(Amazon Managed Service for Prometheus). Ces métriques existaient donc déjà, mais n'étaient pas
...
\begin{table}[h]
	...
	\begin{tabular}{|p{0.2\textwidth}|p{0.25\textwidth}|p{0.45\textwidth}|}
		...
		\hline
		Logs & Tous & Slack (\texttt{\#\$\{TEAM\}-logs-\$\{ENV\}}) \\
		\hline
	\end{tabular}
\end{table}
