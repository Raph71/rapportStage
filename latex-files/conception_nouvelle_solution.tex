...
Le principal levier de la nouvelle solution consiste à déplacer l'alerting des métriques d'infrastructure
vers les métriques d'application. Jusqu'ici, les alertes reposaient presque exclusivement sur des
...
Or la plateforme expose déjà un ensemble riche de métriques applicatives, produites par un middleware
JSON-RPC instrumenté avec OpenTelemetry, puis collectées et stockées dans AMP (Amazon Managed
Service for Prometheus). Ces métriques n'étaient pourtant pas exploitées pour l'alerting. Les brancher
à la chaîne d'alerte constitue le cœur de ma contribution : elles permettent d'alerter au niveau du
métier — le comportement des services tel que les joueurs le perçoivent — et non plus seulement au
niveau des ressources techniques.

Cette bascule est rendue possible par Prometheus et son langage de requêtes, PromQL. Là où
CloudWatch se limite pour l'essentiel à des seuils sur des métriques d'infrastructure, PromQL permet
d'exprimer des conditions fines et directement liées à l'expérience utilisateur. Sur cette base, j'ai
standardisé trois métriques appliquées aux API de la plateforme :

\begin{itemize}
	\item \textbf{Latency P99} : le temps de réponse du 1 \% des requêtes les plus lentes, révélateur d'une lenteur ressentie par les joueurs ;
	\item \textbf{Error Rate} : la proportion de requêtes en échec, calculée à partir du statut JSON-RPC renvoyé par chaque service ;
	\item \textbf{Service Down} : la chute anormale du volume de requêtes reçues par un service, signe qu'il ne répond plus.
\end{itemize}

Prises ensemble, ces métriques traduisent directement l'impact sur l'utilisateur : une hausse du taux
d'erreur sur l'API de dépôt signale immédiatement qu'un joueur ne parvient plus à déposer d'argent —
une information bien plus actionnable qu'un simple pic de CPU. Au-delà de ces trois indicateurs
standards, PromQL offre à chaque équipe la liberté de définir ses propres alertes métier, au plus près
de la réalité de son service.
...
Afin d'améliorer le temps moyen de réparation (MTTR), les alertes RED de criticité « Critical » ou
« High » (les conséquences) sont associées aux alertes USE « Warning » du même service (les causes
possibles). Cette corrélation facilite le diagnostic pour la personne d'astreinte : plutôt que de recevoir
une alerte isolée, elle dispose immédiatement des pistes de cause. Par exemple, si le service
