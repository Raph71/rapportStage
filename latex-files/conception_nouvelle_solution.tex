...
Pour structurer la surveillance, j'ai retenu deux méthodes reconnues de l'ingénierie SRE, en les appliquant à des types de \lexique{métriques} distincts. La méthode USE
s'applique aux \lexique{métriques} d'infrastructure et adopte une vue « côté machine », tandis que la méthode RED s'applique aux \lexique{métriques} d'application et adopte une vue « côté client ».
...
La méthode RED se concentre sur les requêtes applicatives, au plus près de
...
Cette bascule est rendue possible par \lexique{Prometheus} et son langage de requêtes, PromQL. Là où
CloudWatch se limite pour l'essentiel à des seuils sur des \lexique{métriques} d'infrastructure, PromQL permet
...
\begin{itemize}
	\item \textbf{Latency P99} : le temps de réponse maximum pour 99 \% des requêtes ;
...
diagnostiquer et résoudre le problème — et à un lien direct vers le tableau de bord (dashboard)
