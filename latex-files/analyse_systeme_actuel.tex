...
L'architecture étudiée repose sur un ensemble de services \lexique{AWS}. Les applications de Winamax
s'exécutent dans des conteneurs regroupés au sein de clusters \lexique{ECS} (\emph{Elastic Container Service}, le
service d'\lexique{AWS} qui héberge et orchestre les conteneurs). Le fonctionnement observé peut se résumer
ainsi : chaque changement sur un service (création, mise à jour, suppression) est enregistré par
\lexique{CloudTrail} — le journal d'audit qui trace toutes les actions réalisées sur le compte \lexique{AWS} — puis relayé
par \lexique{EventBridge}, le bus d'événements qui déclenche des traitements en réaction à ces changements. Ces
événements exécutent alors des fonctions \lexique{Lambda} (de petits programmes lancés à la demande, sans
serveur à gérer) qui créent ou suppriment des alarmes dans \lexique{CloudWatch}, l'outil de surveillance d'\lexique{AWS}.
Enfin, les alarmes déclenchées émettent des notifications qui, via un service de messagerie (un topic
\lexique{SNS} puis une file d'attente \lexique{SQS}), aboutissent sur les canaux de diffusion, principalement Slack.
...
L'étude détaillée du système m'a conduit à recenser les principales fonctions \lexique{Lambda} qui composent
...
\begin{table}[h]
	...
	\begin{tabular}{|p{0.35\textwidth}|p{0.55\textwidth}|}
		...
		\hline
		cloudwatch-log-groups-stream & Gère automatiquement les filtres d'abonnement CloudWatch Logs et les alarmes pour les groupes de \lexique{logs} ECS, \lexique{Lambda} et Batch, déclenchés par les mises à jour de ressources (événements \lexique{CloudTrail}). \\
		\hline
		cloudwatch-logs-errors-to-slack & Écoute les événements de \lexique{logs} applicatifs, détermine leur pertinence (niveau error), applique des filtres anti faux-positifs, dédoublonne, journalise les \lexique{métriques} au format EMF et publie les \lexique{logs} intéressants vers SNS puis une file SQS. \\
		\hline
...
Le tag appliqué sur les \lexique{target groups} illustre bien la logique de configuration : chaque type d'alarme
...
construite autour de \lexique{métriques} de causes (CPU, mémoire, etc.) plutôt que des conséquences directement
corrélées à un impact utilisateur. Concrètement, le système sait dire « la mémoire de ce conteneur est
...
Un pic de CPU, par exemple, peut être parfaitement normal lors d'un traitement par lots (\emph{batch})
et n'avoir aucun impact sur les joueurs. À l'inverse, une latence anormale sur une API critique peut passer inaperçue si
...
Les alertes sur les causes sont créées automatiquement par les fonctions \lexique{Lambda}. 
...
\begin{table}[h]
	...
	\begin{tabular}{|p{0.45\textwidth}|p{0.45\textwidth}|}
		...
		\hline
		Création automatique des alarmes, zéro intervention manuelle pour surveiller un nouveau service &
		...
		Chaîne trop complexe : plusieurs \lexique{Lambda}s avant l'envoi sur Slack \\
		\hline
		Configuration par tags sans toucher à l'infrastructure &
		Si une \lexique{Lambda} échoue, aucune nouvelle tentative : manque de visibilité \\
		\hline
		 & De nombreuses valeurs écrites en dur dans le code, peu maintenable \\
		\hline
...
\section{Réception des alertes}

Au-delà de la construction des alarmes, j'ai également étudié la manière dont les alertes parviennent
concrètement aux personnes d'astreinte. Aujourd'hui, lorsqu'une alarme \lexique{CloudWatch} jugée critique se
...
\begin{itemize}
	...
	endroits différents (tags de ressources, scripts personnalisés, configuration des \lexique{Lambda}s), sans
	...
	\lexique{Lambda} qui s'exécutent automatiquement en réaction aux événements \lexique{CloudTrail}, sans visibilité ni
