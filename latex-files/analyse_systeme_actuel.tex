...
L'architecture étudiée repose sur un ensemble de services AWS. Les applications de Winamax
s'exécutent dans des conteneurs regroupés au sein de clusters ECS (\emph{Elastic Container Service}, le
service d'AWS qui héberge et orchestre les conteneurs). Le fonctionnement observé peut se résumer
ainsi : chaque changement sur un service (création, mise à jour, suppression) est enregistré par
CloudTrail — le journal d'audit qui trace toutes les actions réalisées sur le compte AWS — puis relayé
par EventBridge, le bus d'événements qui déclenche des traitements en réaction à ces changements. Ces
événements exécutent alors des fonctions Lambda (de petits programmes lancés à la demande, sans
serveur à gérer) qui créent ou suppriment des alarmes dans CloudWatch, l'outil de surveillance d'AWS.
...
construite autour de métriques de causes (CPU, mémoire, etc.) plutôt que des conséquences directement
...
\begin{table}[h]
	...
	\begin{tabular}{|p{0.45\textwidth}|p{0.45\textwidth}|}
		...
		Si une Lambda échoue, aucune nouvelle tentative : manque de visibilité \\
		\hline
