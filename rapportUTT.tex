...
\Kone{Système d'alerting} % Nature de l'activité
\Ktwo{Criticité des alarmes} % Branche d'activité économique
\Kthree{Observabilité} % Domaines technologiques
\Kfourth{Cloud AWS } % Application Physique directe
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\newcommand{\titletext}{
    ...
    Ce stage s'est déroulé dans l'entreprise Winamax, au sein de l'équipe infrastructure. Il a consisté à améliorer
    le système d'alerting afin de passer d'alarmes peu contextualisées à un modèle basé sur plusieurs niveaux de
    criticité, orienté vers le service réellement impacté.

    Les différentes phases de travail successivement réalisées sont :

    \begin{itemize}
        \item Étudier l'existant et dresser une cartographie des alarmes de la plateforme
        \item Définir des niveaux de criticité pertinents pour l'astreinte
        \item Concevoir une stratégie d'alerting centrée sur les symptômes des incidents
        \item Préparer l'évolution technique autour des composants ECS, EventBridge, CloudWatch et Lambda
    \end{itemize}

    L'enjeu est de réduire la fatigue liée aux alertes, d'améliorer la visibilité opérationnelle de la plateforme
    et de permettre une résolution proactive des incidents, avant notification par les utilisateurs.
}
...
\begin{document}
    ...
    % Plan du rapport (version initiale basée sur la spec)
    % 1) Présentation du contexte technique et opérationnel
    % 2) Analyse de l'existant sur la chaîne de création d'alarmes
    % 3) Conception des niveaux de criticité et de la stratégie d'alerting
    % 4) Mise en oeuvre, validation et perspectives
    \import{latex-files/}{presentation.tex}
    \import{latex-files/}{presentation_stage.tex}
    \import{latex-files/}{positionnement.tex}
