...
\Kone{Système d'alerting et observabilité} % Nature de l'activité
\Ktwo{Criticité des alarmes} % Branche d'activité économique
\Kthree{ObservabilitéInfrastructure as Code} % Domaines technologiques
\Kfourth{Cloud AWS, Prometheus, Alertmanager} % Application Physique directe
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Ce stage de fin d'études s'est déroulé au sein de l'équipe Infrastructure de Winamax, leader français
du poker et des paris sportifs en ligne. Il a porté sur la refonte du système d'alerting de la plateforme,
dont les alarmes, historiquement fondées sur des causes techniques telles que le CPU ou la mémoire,
généraient un bruit important et une fatigue d'astreinte. J'ai d'abord cartographié les alarmes
existantes et diagnostiqué les limites de la chaîne actuelle. J'ai ensuite conçu un modèle centré sur les
symptômes, appuyé sur les méthodes RED et USE, quatre niveaux de criticité (P1 à P4) et une matrice
de routage multi-canaux. J'ai développé la solution en Infrastructure as Code avec Terraform, Amazon
Managed Prometheus et Alertmanager, en intégrant runbooks, watchdog et mode dégradé. Cette
refonte vise à ne solliciter l'astreinte que pour des incidents réellement critiques, à réduire le MTTR
et à traiter les problèmes avant leur perception par les utilisateurs.
}
